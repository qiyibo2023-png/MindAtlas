...
G.bind=function(){
 ...
 const form=document.getElementById('global-entry-form');if(form)form.onsubmit=e=>{e.preventDefault();if(!input.value.trim()){entryError(input);input.focus();return;}if(globalThis.SymptomRouter)navigate(SymptomRouter.submit(input.value));else{G.submitText(input.value);navigate('safety');}};
 document.querySelectorAll('[data-global]').forEach(el=>{if(el.dataset.global==='text')return;el.onclick=()=>{if(el.dataset.global==='continue'&&G.acknowledge()){if(['safety','urgent'].includes(view))view=globalThis.SymptomRouter?.store.pending?SymptomRouter.resume():'library';render();}if(el.dataset.global==='reset'){G.reset();globalThis.SymptomRouter?.clear();view='library';render();}};});
};
